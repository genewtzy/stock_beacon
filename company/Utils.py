...
def is_float(string):
    ...
    :return:
    """
    local_string = str(string)
    try:
        if re.search(r'^(-?\d+)(\.\d+)?$',local_string) is None:
            return False
    except ValueError:
        print(run_info(string), " is not a number", type(string))
        return False
    return True
...
def profit_dates_values_sync(date_values: list, dates_sync_to: list, index=0,
                             date_sync=False,profit_type='static_profit', profit_time='async'):
    """
    :param date_sync:
    :param date_values:
    :param dates_sync_to:
    ...
    :param profit_time:'async','sync',指业绩是否和股价同步显示，通常我们看到业绩是会滞后于股价的，滚动情况下是一个季度，静态市盈率情况下
    是最多一年4个月
    """
    ...
    while i < len(sync_date_values):
        ...
        if profit_time == 'async':
            if profit_type == 'static_prifit':
                sync_date = mile_stone_date + relativedelta(years=1) + relativedelta(months=4)
            else:
                sync_date = mile_stone_date + relativedelta(months=4)
        else:
            sync_date = mile_stone_date

        # 通常最迟在第二年的4月30日看到前一年的年报
        if item_date >= sync_date and j < len(date_values):
            mile_stone = date_values[j]
            mile_stone_date = dt.strptime(mile_stone[index], '%Y-%m-%d').date()
            j += 1
        '''
        else:
            if item_date >= mile_stone_date + relativedelta(months=4) and j < len(date_values):
                mile_stone = date_values[j]
...
def json_data_regularize(factor: float, json_data, first_data_zero=False):
    ...
    factor = abs(factor)  #  否则会出现-200比-100增加100的的情况
    i = 0
